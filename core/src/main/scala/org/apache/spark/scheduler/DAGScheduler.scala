...
class DAGScheduler(
  ...
  /**
   * 包含了每个rdd的每个partitions的缓存的位置。这个HashMap的key值是rdd的id，而value值是一个有partition的id
   * 索引的数组。数组中的每个元素表示一组该partition缓存的位置。
  ...
  def getCacheLocs(rdd: RDD[_]): IndexedSeq[Seq[TaskLocation]] = cacheLocs.synchronized {
    ...
    if (!cacheLocs.contains(rdd.id)) {
      ...
      val locs: IndexedSeq[Seq[TaskLocation]] = if (rdd.getStorageLevel == StorageLevel.NONE) {
        ...
        // 再通过请求blockManagerMaster，获取location信息（如果该rdd的partition计算还没完成，
        // 则返回空）
        // 这个rdd的blocks是在shuffle writer中存储的（在内存中存储或这spill的时候存储到磁盘），
        // 然后在master登记的。所以，如果locs为空，说明该rdd所在的stage的task还没有计算完成。
        // 需要再看看shuffle writer的代码，然后把两部分联系起来。(不对，可能理解的不对!!)
        // （问题是shuffle writer怎么知道我的rdd.id??? 或者我们的rdd shuffle writer是不知道的，
        // 但是，它知道我们的rdd的shuffleDependency的对应的parent rdd的Block信息？然后我们的rdd通过
        // getPreferredLocsInternal来间接的获取该location信息???）
  ...
  private def createResultStage(
    ...
    val parents = getOrCreateParentStages(rdd, jobId)
    val id = nextStageId.getAndIncrement()
    // 在这里创建ResultStage，是不是意味着，在这之前所有其它的ShuffleMapStage都创建好了???
  ...
  /**
   * 这个函数会通过getShuffleDependencies()一层一层往上
   * 找到所有还未在shuffleIdToMapStage里注册的ShuffleDependency。注意，只找ShuffleDependency
   * Find ancestor shuffle dependencies that are not registered in shuffleToMapStage yet
   */
  private def getMissingAncestorShuffleDependencies(
      rdd: RDD[_]): ArrayStack[ShuffleDependency[_, _, _]] = {
    // 注意ancestors是一个stack，先进后出，所以我们之后在遍历ancestors的时候，
    // 就会优先创建处于上流的stage（即肯定是先创建parent stage，再创建child stage）。
    ...
    while (waitingForVisit.nonEmpty) {
      ...
      if (!visited(toVisit)) {
        ...
        // TODO 感觉这边可以把rdd之间的"直接shuffle dependency关系"保存起来(比如Map[id, HashSet[dep]])，
        // 这样，在createShuffleMapStage#L359时，就不用再重新找直接地shuffleDependencies了。
        // 寻常toVisit（rdd）的ShuffleDependencies
        getShuffleDependencies(toVisit).foreach { shuffleDep =>
          // 如果该ShuffleDependency还未在shuffleIdToMapStage中注册
  ...
  private[scheduler] def getShuffleDependencies(
    ...
    // FIXME 感觉可以把一个rdd的直接ShuffleDependency存储下来，这样之后再调用就不要用重复找了
    // 但是，rdd如果被checkpoint，则它的dependency会变！！！所以我们需要在checkpoint的时候，
    // 清空该rdd的直接ShuffleDependency缓存 或者 在该放进入时，判断该rdd是否被checkpoint，如果是
    // 则清空它的缓存。
      ...
      if (!visited(toVisit)) {
        ...
        toVisit.dependencies.foreach {
          case shuffleDep: ShuffleDependency[_, _, _] =>
            ...
            // （哈哈，早几个月前自己留下的注释，显然不对。一个RDD怎么可能最多只有一个ShuffleDependency呢???
            // 不过，我在方法上写的注释现在看起来倒是不错。）
            parents += shuffleDep
          case dependency =>
    ...
    // 注意：只返回该rdd直接（所谓的直接看方法上面的注释）的ShuffleDependencies
    parents
  }
  ...
  private def getMissingParentStages(stage: Stage): List[Stage] = {
    ...
    def visit(rdd: RDD[_]) {
      if (!visited(rdd)) {
        ...
        // rdd有未被缓存的分区？？？
        // 如果为true，说明该rdd所在的stage的tasks还没有计算完成，所以我们需要
        // 提交该stage（当然还是需要先找找有没有未完成的parent stages)
          ...
          for (dep <- rdd.dependencies) {
            dep match {
              case shufDep: ShuffleDependency[_, _, _] =>
                ...
                // 如果该stage还未完成，则加入missing（missing是我们先要完成的parent stages）
  ...
  def submitJob[T, U](
    ...
    // 通过eventProcessLoop提交JobSubmitted事件
  ...
  def runJob[T, U](
    ...
    // 阻塞等待job完成（成功或失败）
    // TODO QUESTION 真的是阻塞吗???难道DAGScheduler同时只能处理一个job吗？但是DAG有一个activeJobs，
    // 看该变量的用法似乎DAG可以同时处理多个job啊。但是，我们如果用rdd到做计算的话，肯定得等到一个job结束，
    // 在得到它的计算结果之后，把该结果作为输入，再开始执行下一个job啊。
    // 可能的解释是，那是MapStage类型的jobs或者spark内部提交的job，比如checkpoint rdd的时候就在内部提交了job。
    // 那是不是说，在user code提交的job是不可能同时有多个在执行的？？？
    ...
    waiter.completionFuture.value.get match {
      ...
      case scala.util.Failure(exception) =>
        logInfo("Job %d failed: %s, took %f s".format
          (waiter.jobId, callSite.shortForm, (System.nanoTime - start) / 1e9))
        // TODO QUESTION 看不懂这个callerStackTrace
  ...
  /**
   * 重新提交所有的failed stages。通常，该方法会等待一小会儿，直到最后一个FetchFailure到达。
  ...
  private[scheduler] def resubmitFailedStages() {
    if (failedStages.size > 0) {
      ...
      failedStages.clear()
      // 执行所有的failed stages
      // （这些failed stage应该没有"直接"的父子关系???）
      for (stage <- failedStagesCopy.sortBy(_.firstJobId)) {
        // 优先执行job id小的stages(体现job调度的FIFO策略)
        submitStage(stage)
      }
    }
  }
  ...
  // 处理作业提交--This is very important start...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    try {
      ...
      case e: Exception =>
        ...
        // 通知JobListener（比如JobWaiter），job执行失败啦
        listener.jobFailed(e)
        return
    }

    // 在finalStage创建完成之后，创建ActiveJob
    ...
    // QUESTION：在这里调用getMissingParentStages()的时候，所有的stages都已经创建好了吗？
    logInfo("Missing parents: " + getMissingParentStages(finalStage))

    val jobSubmissionTime = clock.getTimeMillis()
    // 添加jobId到job对象之间的映射
    jobIdToActiveJob(jobId) = job
    // 将该job添加到activeJobs的集合中
    // TODO QUESTION：在runJob()中会阻塞等待一个job结束。
    // 所以，DAGScheduler中怎么会同时有多个activeJobs??? 可能有MapStage类型的Job提交???
    activeJobs += job
  ...
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        // 如果所有parent stages都已经完成了，就直接提交该stage中tasks
        if (missing.isEmpty) {
          ...
          // 如果有parent stage还没有执行，则优先执行parent stage，
          // 并该stage暂时存储发哦waitingStages中
          waitingStages += stage
        }
      }
    } else {
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    val taskIdToLocations: Map[Int, Seq[TaskLocation]] = try {
      stage match {
        case s: ShuffleMapStage =>
          // 注意，在这里，taskId和partition的id还是对应的
      ...
      case NonFatal(e) =>
        // 如果有NonFatal的异常抛出，则直接abort该stage（意味着该job也就直接挂咯）
    ...
    try {
      ...
      val taskBinaryBytes: Array[Byte] = stage match {
        case stage: ShuffleMapStage =>
          // 对于ShuffleMapStage，我们序列化它的shuffleDep，这样该stage就能知道如何去
          // 获取上一个stage的outputs
          JavaUtils.bufferToArray(
            closureSerializer.serialize((stage.rdd, stage.shuffleDep): AnyRef))
        case stage: ResultStage =>
          // 对于ResultStage，我们序列化它的func，该func用于对倒数第二个stage的output做计算，
          // 并生成最终的结果
    ...
    val tasks: Seq[Task[_]] = try {
      ...
      stage match {
        case stage: ShuffleMapStage =>
          // 重新设置该stage的pendingPartitions
          stage.pendingPartitions.clear()
          partitionsToCompute.map { id =>
            // taskIdToLocations可能为空啊
            // locs是集群节点的位置（host、executor）信息
            val locs = taskIdToLocations(id)
            // part是rdd的某个partition
            val part = stage.rdd.partitions(id)
            // 更新pendingPartitions，说明需要等待该id对应的part计算完成
        ...
        case stage: ResultStage =>
          partitionsToCompute.map { id =>
            ...
            val locs = taskIdToLocations(id)
            // 构建ResultTask(和ShuffleMapTask比多了一个id，这个id就是该ResultTask所要读取的output的分区id)
      ...
      // 如果在构建task的过程中抛出NonFetal的异常，则也直接abort该stage
      case NonFatal(e) =>
        abortStage(stage, s"Task creation failed: $e\n${Utils.exceptionString(e)}", Some(e))
        runningStages -= stage
        return
    ...
    if (tasks.size > 0) {
      ...
      // 如果没有task需要执行（且该stage结束），则检查其它需要被执行的child stages
        ...
        task match {
          case rt: ResultTask[_, _] =>
            ...
            resultStage.activeJob match {
              case Some(job) =>
                // 为每个result task只更新一次accumulator
                // Only update the accumulator once for each result task.
                if (!job.finished(rt.outputId)) {
                  // TODO read updateAccumulators
    ...
    event.reason match {
      case Success =>
        task match {
          case rt: ResultTask[_, _] =>
            ...
            resultStage.activeJob match {
              case Some(job) =>
                // 如果outputId对应的分区的计算还没完成，那么我们现在标记它已完成
                if (!job.finished(rt.outputId)) {
                  job.finished(rt.outputId) = true
                  // 已经完成的result task个数+1
                  job.numFinished += 1
                  // 如果整个job都已经完成了
                  // If the whole job has finished, remove it
                  if (job.numFinished == job.numPartitions) {
                    // 先标记该ResultStage为结束状态
                    markStageAsFinished(resultStage)
                    // 清理和该job相关的数据结构（例如，当该job中某个stage只被该job使用时，
                    // 则将其从DAGScheduler中清除）
                  ...
                  try {
                    // 通知job listener（比如，JobWaiter），outputId对应分区的计算结果拿到啦！
      ...
      case Resubmitted =>
        ...
        stage match {
          ...
          case _ =>
            // TaskSerManagers只能resubmitted ShuffleMapStage里的tasks（为啥???）
      ...
      case FetchFailed(bmAddress, shuffleId, mapId, reduceId, failureMessage) =>
        ...
        if (failedStage.latestInfo.attemptNumber != task.stageAttemptId) {
          ...
          // 我们无法获取该map stage的output，则我们把mapId对应的task在mapOutputTracker的注册
          // MapOutput清理掉(反正这些信息也不能帮我们正确的获取到数据，而我们之后resubmit的stage
          ...
          if (bmAddress != null) {
            ...
            // 移除该executor，以及清除在master上登记的outputs
